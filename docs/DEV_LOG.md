...
## 2026-04-12 ~00:18 — Web track() beacon utility (TDD)

**작업 내용:**
- `web/lib/track.ts` 구현: `sendBeacon(url, JSON.stringify(payload))` 래퍼.
- `web/__tests__/track.test.ts` 3개 테스트 작성 후 TDD 절차 준수 (Red → Green).
- 개발 환경(`NODE_ENV === 'development'`)에서는 `sendBeacon` 대신 `console.debug`로 폴백.
- SSR/구형 브라우저 대비 `typeof navigator === 'undefined'` 가드 포함.

**기술적 결정:**

### 1. sendBeacon body를 JSON 문자열로 전달
- **결정:** `navigator.sendBeacon(url, JSON.stringify(payload))` — body를 Blob 없이 plain string으로.
- **이유:** body가 string이면 Content-Type이 `text/plain;charset=UTF-8`로 전송되어 CORS preflight가 발생하지 않음. Workers의 `handleTrack`이 `text/plain` body를 수신한다.

### 2. value 필드를 undefined일 때 payload에서 제외
- **결정:** `if (value !== undefined) payload.value = value` — 조건부 추가.
- **이유:** JSON 직렬화 시 `"value":undefined`는 자동 제거되지만, 명시적으로 제외해야 테스트의 JSON 문자열 비교가 정확히 일치함.

**검증 결과:** 3 tests ALL PASS.

---

## 2026-04-12 ~24:17 — fetch() restructure with /e endpoint and redirect tracking sampling

**작업 내용:**
- `workers/src/index.ts`의 `fetch()` 핸들러를 재구성하여 추적 기능 통합.
- 서브도메인 't' + 경로 '/e'에서 POST → `handleTrack()`, OPTIONS → CORS preflight 204 반환.
- 캐시 히트/미스 양쪽에서 302 리다이렉트 시 10% 샘플링으로 `writeEvent("redirect", keyword)` 호출.
- 서브도메인 't'는 리다이렉트 추적에서 제외 (A5 Amendment).
- `writeEvent`와 `cache.put`을 별도의 `ctx.waitUntil()` 호출로 분리하여 에러 격리.
- 6개의 새 테스트 추가 (Tracking endpoint 3개 + Redirect sampling 3개).

**기술적 결정:**

### 1. 서브도메인 이중 추출 유지
- **결정:** `fetch()`에서 서브도메인을 추출하고, `_fetch_keyword()`에서도 독립적으로 다시 추출.
- **이유:** `_fetch_keyword`의 기존 동작을 변경하지 않고 안전하게 추적 기능을 추가하기 위함. 리팩토링 리스크 회피.

### 2. 캐시 로직 전에 /e 분기
- **결정:** 추적 엔드포인트를 캐시 조회보다 먼저 처리.
- **이유:** POST /e 요청은 캐시와 무관하며, 빠른 응답을 위해 캐시 로직을 거칠 필요 없음.

### 3. 10% 샘플링을 `Math.random()`으로 구현
- **결정:** 각 요청마다 `Math.random() < 0.1`로 샘플링.
- **이유:** D1 무료 티어 쓰기 제한(100K writes/day) 내에서 운영 가능. 통계적으로 충분한 데이터 확보.

**검증 결과:** 42 workers tests + 10 web tests + 23 scripts tests = 75 tests ALL PASS.
...
**작업 내용:**
Tier 1 위트형 "가격" 재해석 키워드를 ~80개에서 ~125개로 확장. 총 45개 신규 키워드 추가.
...
**검증:** 52개 테스트 ALL PASS, 총 1046개 키워드 파일

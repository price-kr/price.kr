...
| 8 | 리뷰 피드백 반영 (iter 1) | ✅ 완료 | clearing→typing 200ms 지연, role="button" 모순 제거, JetBrains Mono CDN, 모바일 @media 이식 |
| 9 | 리뷰 피드백 반영 (iter 3–7, /loop self-paced) | ✅ 완료 | onBlur guard, googleapis preconnect, 모바일 자식 셀렉터, loadData 중복 제거, NaN 가드, inputRef 제거, paddingRight, focus-visible 링 + scope, overflow-x-hidden, eyebrow `<p>`, stepper `<ol>`, microcopy 중립화 |
| 10 | `[keyword]` 3가지 뷰 Concept A 디자인 적용 | ✅ 완료 | SiteHeader/SiteFooter 추출, alias/canonical/unregistered 뷰 새 토큰 + display 헤드라인 + EyebrowPill/MonoUrl/PrimaryCTA helper, FloatingChips excludeKeyword 적용 |

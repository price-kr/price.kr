...
| # | Task | 상태 | 비고 |
|---|------|------|------|
| 1 | D1 Migration SQL + Blocklist 't' | ✅ 완료 | events 테이블 스키마, 't' 서브도메인 블로킷 추가 |
| 2 | writeEvent function + tests (TDD) | ✅ 완료 | workers/src/tracking.ts, D1 INSERT, 5 tests |
| 3 | handleTrack + corsHeaders + tests (TDD) | ✅ 완료 | POST /e 핸들러, CORS, 6 tests |
| 4 | fetch() restructure + redirect sampling | ✅ 완료 | /e 엔드포인트 분기, 10% 샘플링, 6 tests |
| 5 | Web track() utility + tests (TDD) | ✅ 완료 | web/lib/track.ts, sendBeacon 래퍼, dev 모드 콘솔 폴백, 3 tests |
| 6 | PageTracker component + page insertions | ✅ 완료 | "use client" component, useRef StrictMode guard, home + keyword page |
| 7 | SearchBar tracking + privacy page update | ✅ 완료 | handleSelect closure + [query] deps, privacy IP 문구 수정 |
| 8 | Full test suite + lint verification | ✅ 완료 | 78 tests all pass (42 workers + 13 web + 23 scripts) |
| 9 | DEV_PROGRESS.md + DEV_LOG.md update | ✅ 완료 | |
